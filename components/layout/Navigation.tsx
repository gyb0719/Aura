  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? "bg-white/95 dark:bg-gray-900/95 backdrop-blur-lg shadow-sm" : "bg-transparent"
      }`}
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          <div className="hidden md:flex items-center gap-6 lg:gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm lg:text-base text-gray-700 dark:text-gray-300 hover:text-pink-600 dark:hover:text-pink-400 transition-colors duration-200 font-medium"
                onClick={(e) => {
                  e.preventDefault()
          ...
          <div className="hidden md:flex items-center gap-3 lg:gap-4">
            ...
            <Link
              href="/auth/signin"
              className="px-4 py-2 text-sm lg:text-base text-gray-700 dark:text-gray-300 hover:text-pink-600 dark:hover:text-pink-400 transition-colors font-medium"
            >
              로그인
            </Link>
            <Link
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                <button
                  ...
                  className="w-full py-2 text-gray-700 dark:text-gray-300 hover:text-primary transition-colors"
                >
                  로그인
                </button>
                <button
